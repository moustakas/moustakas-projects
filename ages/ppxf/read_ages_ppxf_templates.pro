function read_ages_ppxf_templates, out_tempwave, velscale=velscale, $
  ...
  tempinfo=out_tempinfo, solar=solar, bc03=bc03

; jm09nov12ucsd - read the output from BUILD_AGES_PPXF_TEMPLATES and
;   optionally resample to a constant ln-lambda; also optionally
;   convolve to the fiducial AGES instrumental velocity dispersion;
;   note that INST_VDISP *requires* VELSCALE; also, if VELSCALE is
;   passed then OUT_TEMPWAVE is in natural log
; jm10jun14ucsd - added /SOLAR
; jm16feb10siena - added CKC14z templates
...
;   common ppxf_templates, tempflux, tempwave, temphdr, tempinfo
    ...
    if (n_elements(tempflux) eq 0) then begin
       if keyword_set(bc03) then begin
          ...
          if keyword_set(solar) then metal = '0.0190' else $
            metal = ['0.0049','0.0190','0.0300']
          templatefile = ages_path(/ppxf)+'ckc14z_'+metal+$
            '_'+ages_version(/ppxf_templates)+'.fits.gz'
       endelse
...
;         tempwave = float(10D^make_wave(temphdr))
...
;       model_inst_vdisp = sxpar(temphdr,'VDISP')
;       vdisp = sqrt(inst_vdisp^2.0-model_inst_vdisp^2)>0.0
