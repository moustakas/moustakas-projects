...
function mz_kcorrect_restore, info, vname=vname
...
; frame  
    ...
    for igal = 0L, ngal-1L do begin
       model[igal].wave = wave*(1.0+info[igal].k_z) ; [A]
       model[igal].flux = model[igal].flux/(1.0+info[igal].k_z)
    endfor
...
pro mz_kcorrect_qaplot, info, psfile=psfile, vname=vname, $
...
;     in_filterlist = strtrim(info[0].filterlist,2)
    ...
    model = mz_kcorrect_restore(info,vname=vname)
