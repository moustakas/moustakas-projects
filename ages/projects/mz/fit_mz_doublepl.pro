function fit_mz_doublepl, mass, oh, weight, oh_err=oh_err, binsize=binsize, $
  minmass=minmass, maxmass=maxmass, mingal=mingal, fit_minmass=fit_minmass, $
  verbose=verbose
; jm10aug13ucsd - fit the MZ relation using a smoothly connected
; double power-law (see, e.g., Barger+05, Aird+08)
...
;   splog, 'Fitting binned points'
    coeff = mpfitfun('mz_doublepl',mzbin.xbin,mzbin.medy,$
    ...
    chi2 = chi2/float(dof)
    scatter = djsig(oh-mz_doublepl(mass,coeff))
...
;   splog, 'Fitting all points'
    ...
    coeff_all = mpfitfun('mz_doublepl',mass,oh,$
    ...
    scatter_all = djsig(oh-mz_doublepl(mass,coeff_all))
