function fit_mz_poly, mass, oh, weight, oh_err=oh_err, binsize=binsize, $
  minmass=minmass, maxmass=maxmass, mingal=mingal, fit_minmass=fit_minmass, $
  verbose=verbose
; jm10aug13ucsd - fit the MZ relation using a simple polynomial
...
;   splog, 'Fitting binned points'
    coeff = mpfitfun('mz_poly',mzbin.xbin,mzbin.medy,$
    ...
    chi2 = chi2/float(dof)
    scatter = djsig(oh-mz_poly(mass,coeff))
...
;   splog, 'Fitting all points'    
    ...
    coeff_all = mpfitfun('mz_poly',mass,oh,$
    ...
    scatter_all = djsig(oh-mz_poly(mass,coeff_all))
