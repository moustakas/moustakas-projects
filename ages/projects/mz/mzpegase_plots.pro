...
;     /left, /top, box=0, charsize=1.4, margin=0
    im_legend, '\beta = '+string(beta,format='(F3.1)'), $
      /right, /bottom, box=0, charsize=1.4, position=[10.8,8.45], /data
    im_legend, ['\alpha = ','','','']+string(alpha,format='(F4.1)'), /bottom, $
...
; sfrm vs mass    
    plot, [0], [0], /nodata, /noerase, xrange=massrange, yrange=sfrmrange, $
      xsty=1, ysty=1, xtitle=mzplot_masstitle(), ytitle=textoidl('sSFR (Gyr^{-1})'), $
...
; oh vs mass
    ...
    im_legend, '\alpha = '+string(alpha,format='(F4.1)'), /left, $
...
; sfrm vs mass
    plot, [0], [0], /nodata, /noerase, xrange=massrange, yrange=sfrmrange, $
      ...
      ytickinterval=0.5
    axis, /yaxis, yrange=sfrmrange, ysty=1, ytitle=textoidl('sSFR (Gyr^{-1})')
    djs_oplot, maxis2, poly(maxis2,[alog10(1.29)-10.0*0.77,0.77-1.0])+9, line=0, $
      thick=6, color=im_color('dark violet')
    djs_oplot, maxis2, poly(maxis2,[-6.33,-0.35])+9, line=0, thick=6, color=im_color('orange')
    im_legend, ['Salim+07','Elbaz+07'], /left, /bottom, line=0, thick=6, $
...
;   ytitle1 = '\Delta'+' [12 + log (O/H)]'
    ytitle2 = textoidl('sSFR (Gyr^{-1})') ; mzplot_sfrmtitle()
...
; metallicity evolution    
    ...
    for mm = nmstar-1, 0, -1 do begin
       ...
       oploterror, zaxis2+zoff[mm], mzbest, mzhi-mzbest, color=im_color(mstarcolor[mm]), psym=symcat(mstarpsym[mm]), $
         symsize=mstarsymsize[mm], errcolor=im_color(mstarcolor[mm]), /hibar
...
;        symsize=mstarsymsize[mm]*0.8; line=mstarline[mm], thick=8
...
; sfrm evolution
    djs_plot, [0], [0], /nodata, /noerase, ysty=1, xsty=1, position=pos[*,1], $
      ...
      ytickinterval=1.0
    im_legend, mstarlabel, /right, /bottom, charsize=1.8, line=mstarline, $
...
; salim+07    
    ...
    for mm = 0, nmstar-1 do djs_oplot, [0.1], [poly(mstar[mm],[-6.33,-0.35])+9], $
      ...
      psym=symcat(mstarpsym[mm],thick=6), symsize=mstarsymsize[mm]
;     psym=symcat(psym_salim,thick=6), symsize=symsize_salim

; Noeske+07, from Dunne+1-
    psym_noeske = 9
    symsize_noeske = 2.5
    zz = [0.36,0.58]
;   zz = [0.36,0.58,0.78]
...
;      sfrm = alog10([2.3,4.0,5.5]*(10D^mstar[mm]/1D10)^0.67)-mstar[mm]
       djs_oplot, zz, sfrm+9, color=im_color(mstarcolor[mm]), $
         psym=symcat(mstarpsym[mm],thick=6), symsize=mstarsymsize[mm]
;        psym=symcat(psym_noeske,thick=6), symsize=symsize_noeske
    endfor
...
; Karim+11
    ...
    djs_oplot, zz, alog10(sfr)-mass+9, $ ; sfrerr/sfr/alog(10), $ ; 10^10
      ...
      psym=symcat(mstarpsym[mm],thick=6), symsize=mstarsymsize[mm]
;     psym=-symcat(psym_karim,thick=6), symsize=symsize_karim
