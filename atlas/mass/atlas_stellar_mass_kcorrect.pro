;+
; NAME:
;       ATLAS_STELLAR_MASS_KCORRECT
;
; PURPOSE:
;       Compute stellar masses for the ATLAS using Blanton's
;       k-correct.  
;
; COMMENTS:
;
...
pro atlas_stellar_mass_kcorrect, atlas, atlasinfo, bigresult, debug=debug, write=write

    outpath = atlas_path(/analysis)
    outfile = 'atlas_stellar_mass_kcorrect.fits'
...
; initialize the output data structure
    ...
    redshift = atlas.distance*h100*100.0/light
...
;               maggies_flam = maggies*10^(-0.4*48.6)*light2/rebin(filtinfo.weff,nfilter,ngalaxy)^2
                ...
                plot, [0], [0], /nodata, xsty=3, ysty=3, charsize=1.5, charthick=2.0, xthick=2.0, $
                  ...
                  xrange=xrange, title=strtrim(atlas[i].galaxy,2)
