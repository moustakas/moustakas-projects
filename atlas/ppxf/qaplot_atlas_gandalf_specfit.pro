...
; page2 - emission-line plot
    ...
    xrange = xrange+rebin(50.0*[-1,1],2,npanel)
...
pro qaplot_atlas_gandalf_specfit, specdata, nuclear=nuclear, $
  solar=solar, psfile=psfile
; jm10mar04ucsd - build a QAplot from the ATLAS_GANDALF_SPECFIT output
    ...
    if (n_elements(psfile) eq 0) then psfile = specfitpath+$
      'qaplot_atlas_'+suffix+'_'+version+'.ps'

    if (n_elements(specdata) eq 0) then $
      specdata = read_atlas_gandalf(nuclear=nuclear,solar=solar)
    specfit = read_atlas_gandalf_specfit(specdata,nuclear=nuclear,$
      solar=solar)
