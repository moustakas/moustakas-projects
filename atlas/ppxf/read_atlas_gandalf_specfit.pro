function read_atlas_gandalf_specfit, specdata, unfluxed=unfluxed, $
  test=test, observed=observed, linear=linear, silent=silent, $
  nuclear=nuclear, solar=solar
; jm10mar04ucsd - based on READ_ATLAS_SPECFIT; default is to return
...
;   observed frame, and /LINEAR to convert to linear wavelength
    ...
    if (nobj eq 0) then begin
       doc_library, 'read_atlas_gandalf_specfit'
       return, -1
    endif
    ...
    specfitfile = specfitpath+'atlas_specfit_'+suffix+'_'+version+'.fits.gz'
...
; read the SPECFIT file       
    ...
    dwave = atlas_pixel_size() ; [A/pixel] (see /linear)
