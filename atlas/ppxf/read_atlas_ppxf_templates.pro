function read_atlas_ppxf_templates, out_tempwave, velscale=velscale, $
  inst_vdisp=inst_vdisp, ntemp=ntemp, npix=ntemppix, $
  linear_tempwave=linear_tempwave, linear_tempflux=linear_tempflux, $
  tempinfo=out_tempinfo
; jm09dec17ucsd - read the output from BUILD_ATLAS_PPXF_TEMPLATES and
;   optionally resample to a constant ln-lambda; also optionally
;   convolve to the fiducial ATLAS instrumental velocity dispersion;
...
;   passed then OUT_TEMPWAVE is in natural log
    ...
    if (n_elements(tempflux) eq 0) then begin
       templatefile = atlas_path(/ppxf)+'atlas_ppxf_bc03_templates_'+$
         atlas_version(/ppxf_templates)+'.fits.gz'
       splog, 'Reading '+templatefile
       tempflux = mrdfits(templatefile,0,temphdr,/silent)
       tempwave = float(make_wave(temphdr))
       tempinfo = mrdfits(templatefile,1,/silent)
    endif
...
;      lnflux1 = im_log_rebin(linear_tempwave,linear_tempflux[*,0],$
;        vsc=velscale,outwave=out_tempwave)
       log_rebin, minmax(linear_tempwave), tempflux[*,0], $
         lnflux1, out_tempwave, velscale=velscale
;      jj = rebin_spectrum(tempflux[*,0],linear_tempwave,$
;        exp(lnwave2))
       ntemppix = n_elements(lnflux1)
       out_tempflux = fltarr(ntemppix,ntemp)
       out_tempflux[*,0] = lnflux1
       for ii = 1, ntemp-1 do begin
;         lnflux1 = im_log_rebin(linear_tempwave,$
;           linear_tempflux[*,ii],vsc=velscale)
          log_rebin, minmax(linear_tempwave), tempflux[*,ii], $
            lnflux1, velscale=velscale
          out_tempflux[*,ii] = lnflux1
       endfor
    endif

; convolve to the ATLAS instrumental resolution
...
;           fwhm=2.35*smoothing,/norm,ndim=1)
          ...
          for ii = 0, ntemp-1 do out_tempflux[*,ii] = $
            convol(out_tempflux[*,ii],kernel,/edge_truncate)
       endif
    endif
