...
;
; PURPOSE:
;       Mosaic all the IR-selected spectra spectra with the DSS
;       visualizations.   
;
; CALLING SEQUENCE:
;
...
pro ir_spectrum_mosaic, atlas, postscript=postscript, make_png=make_png
; jm05aug12uofa 
    
; initialize path names

    if keyword_set(make_png) then begin
       pspath = atlas_path(/projects)+'irgalaxies/'
       postscript = 0L
       labelcolor = 'white'
       speccolor = 'white'
       no_specfit = 1L
...
; restore all the fitting results
    ...
    if (n_elements(atlas) eq 0L) then atlas = read_irgalaxies()

    srt = sort(atlas.ir_lum)
    atlas = atlas[srt]
...
;      /landscape
    ...
    psname = 'ir_figure_'+string(lindgen(npage),format='(I2.2)')+'.ps'
...
;            ytitle = 'Normalized Flux'
          ...
          if (indx[0] lt ngalaxy) then begin
             atlas_display_image, atlas[indx], imagepath=dsspath, imposition=pos[*,2*k], $
               ...
               pspath=pspath, labeltype=2L, _extra=extra, spacing=0, $ ;postscript=postscript, $
