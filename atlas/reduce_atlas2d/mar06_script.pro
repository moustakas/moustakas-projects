pro mar06_script, doplot=doplot, preproc=preproc, zptshift=zptshift, web=web
; jm06jan05uofa

    dateroot = '06mar'
    dates = dateroot+['24','25']
    ...
    hiicalibfile = 'caliblist_hii_'+dates+'.txt'
...
; master sky flat          
          ...
          skyflats = file_search(root[iroot]+'0[1-5].fits')
...
;      ispec_makelists, 'a.'+fileroot, dates, $
;        /batchfile, gzip=0, lampname='HeAr', extfile='kpnoextinct.dat', gain='2.1', $
;        rdnoise='5.5', trim='5 1199 4 141', overscan='1207 1218 4 141', pscale='1.66666', $
;        minwave_guess='3585', minwave_out='3590', dwave='2.75', badpixfile='badpix.dat', $
;        tracename='', biasfile=biasname, domefile=domename, skyfile=skyflatname, $
;        sensname='sens_4.5_'+dates+'.fits', /all
...
; ---------------------------------------------------------------------------
; carry out all the initial reductions - trace the second night
; ---------------------------------------------------------------------------
    
    iall, paramfile, procfile=procfile, tracefile=tracefile, doplot=doplot

;   readcol, tracefile[0], tracelist, format='A', comment='#', /silent
;   ibatch, paramfile[0], tracelist=tracelist, /distortion
...
;   ibatch, paramfile[0], crsplitfile=crsplitfile[0], /find_pixshift, /debug, /crsplits
    iall, paramfile, crsplitfile=crsplitfile, crfile=crfile, /find_pixshift, rednight=0
    iall, paramfile, crsplitfile=crsplitfile, crfile=crfile, rednight=1 ; PIXSHIFT not necessary

; ---------------------------------------------------------------------------    
; SINGS: stitch; select sky apertures and sky-subtract
; ---------------------------------------------------------------------------    

    sings2d_stitch, 'cra.09'+['13','11','09','15','17','19']+'_2.fits', outname='wcra.0913_2_stitch.fits', $
      object='NGC3031 drift56', scalefactor=[1.05d,1.01,1.0,1.0,1.05,1.08], $
      user_pixshift=[111.0,102.0,109.0,107.0,110.0], wmapname='wmap_ra.0908.idlsave', $
      refindx=2L, minwave=3645.0, dwave=2.75, debug=1, wfits=1    

    sings2d_stitch, 'cra.09'+['28','26','30']+'_2.fits', outname='wcra.0926_2_stitch.fits', $
      object='NGC4631 drift56', scalefactor=[1.13,1.0,1.00], $
      user_pixshift=[110.0,117.0], wmapname='wmap_ra.0934.idlsave', $
      refindx=1L, minwave=3645.0, dwave=2.75, debug=1, wfits=1
...
; ---------------------------------------------------------------------------    
; select sky apertures and sky subtract; do not sky subtract the HII
; regions 
; ---------------------------------------------------------------------------    
...
; generate sensitivity curves for each night (2.5")
    ...
    senstitle = '2006 March '+['24','25']+' (2.5" Slit)'
    iall, paramfile, stdfile=stdfile2_5, senstitle=senstitle, sensinfo=sensinfo, $
      slit_width=2.5, grey=2, doplot=doplot, sensname=senslist, tracename=''
...
; generate sensitivity curves for each night (4.5")
    ...
    senstitle = '2006 March '+['24','25']+' (4.5" Slit)'
    iall, paramfile, stdfile=stdfile4_5, senstitle=senstitle, sensinfo=sensinfo, $
      slit_width=4.5, grey=2, doplot=doplot, tracename=''
...
; make the combined sensitivity functions
    ...
    sensname = 'sens_2.5_'+dateroot+'.fits' & senstitle='2006 March  (2.5" Slit)'
...
;     senstitle=senstitle,doplot=1,/write,slit_width=2.5,zptshift=0.0)
    ...
    sensname = 'sens_4.5_'+dateroot+'.fits' & senstitle='2006 March (4.5" Slit)'
...
; ---------------------------------------------------------------------------    
; flux-calibrate; rectify the HII-region observations (which have not
; been sky-subtracted)
; ---------------------------------------------------------------------------    

    sensname = 'sens_4.5_'+dateroot+'.fits'
    iall, paramfile, fluxcalibfile=calibfile, calibfile=calibfile, $
      sensname=sensname, rednight=0;, tellfits=tellfits, tracename=''

    sensname = 'sens_4.5_'+dateroot+'.fits'
    iall, paramfile, calibfile=hiicalibfile, sensname=sensname;, tellfits=tellfits
...
; compare the sensitivity curves
    ...
    label = [['March 24','Mostly clear'],['March 25','Early clouds, then clear'],['2.5" Slit','']]
    senslist = ['sens_4.5_'+dates+'.fits','sens_2.5_'+dateroot+'.fits']
    meansens = 'sens_4.5_'+dateroot+'.fits'
    isenscompare, senslist, meansens=meansens, label=label, title='2006 March', $
      psname='qaplot_sens_compare_'+dateroot+'.ps', /postscript

    loglist = ['qalog_sens_4.5_'+dates+'.log','qalog_sens_'+['2.5','4.5']+'_'+dateroot+'.log']
    label = [['March 24','Mostly clear'],['March 25','Early clouds, then clear'],['2.5" Slit',''],['4.5" Slit','']]
    iqaphoto, loglist, label=label, title='2006 March', psname='qaplot_photo_'+dateroot+'.ps', /postscript
