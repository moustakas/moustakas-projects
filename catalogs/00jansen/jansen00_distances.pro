...
;
; PURPOSE:
;       Compile distances for the JANSEN00 galaxies.
;
; CALLING SEQUENCE:
;       jansen00_distances, distdata, /write
;
; INPUTS:
;
; OPTIONAL INPUTS:
...
pro jansen00_distances, distdata, write=write, postscript=postscript
    ...
    jansen00 = mrdfits(path+root+'_ned.fits.gz',1,/silent)
    ngalaxy = n_elements(jansen00)

    ra = 15.0*im_hms2dec(jansen00.ra)
    dec = im_hms2dec(jansen00.dec)
...
;     nedgalaxy:           ' ', $
    ...
    distdata.galaxy = strtrim(jansen00.galaxy,2)
;   distdata.nedgalaxy = strtrim(jansen00.ned_galaxy,2)
    distdata.ra = jansen00.ra
    distdata.dec = jansen00.dec

    goodz = where(jansen00.z gt -900)
    distdata[goodz].cz = jansen00[goodz].z*light
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(write) then begin
       outfile = '00jansen_distances.fits'
