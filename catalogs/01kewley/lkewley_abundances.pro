...
;
; PURPOSE:
;	Calculate metal abundances based on a new equation using both
;	[NII]/[OII] (for z > 0.5) and R23 (for z < 0.5). 
;
; CALLING SEQUENCE:
;	abundance = lkewley_abundances(lineflux)
;
; INPUTS:
;	lineflux - data structure with the following fields
;	  ID      
;	  OII_3727
;	  H_beta
;	  OIII_4959
...
;	
; KEYWORD PARAMETERS:
;	noextcor - do not extinction correct
;
; OUTPUTS:
;
;
...
;
; COMMENTS:
;	Unmeasured line fluxes should be set to zero.  
;
; EXAMPLE:
;
;
...
;	J. Moustakas, 2002 May 29, U of A
;	this program was written by L. Kewley (CFA) but the I/O was
;	modified (originally called Neweq_abund_fin)
;-

function lkewley_abundances, lineflux, noextcor=noextcor

    num = n_elements(lineflux)
    if num eq 0L then begin
       print, 'Syntax - abundance = lkewley_abundances(lineflux,noextcor=noextcor)'
       return, -1L
    endif

; now rename the variables to correspond to Lisa's convention    

    name = floor(lineflux.id)
    OII3727_raw = lineflux.OII_3727
    Hb_raw = lineflux.H_BETA
    OIII4959_raw = lineflux.OIII_4959
    OIII5007_raw = lineflux.OIII_5007
    OI6300_raw = lineflux.OI_6300
    Ha_raw = lineflux.H_ALPHA
    NII6584_raw = lineflux.NII_6584
    SII6717_raw = lineflux.SII_6717
    SII6731_raw = lineflux.SII_6731
    SIII9069_raw = lineflux.SIII_9069
    SIII9532_raw = lineflux.SIII_9532
...
;if red_corr eq 'yes' then begin 
    if n_elements(noextcor) eq 0L then begin
       for i=0,num-1 do begin
       ...
       if (logOIII5007OII(i) ne 0.0) and (logR23(i) ne 0.0) then begin
          ...
          for iter=1,n_iter do begin ; iterate if tolerance level not met
             if abs(R23_Z(i,iter)-R23_Z(i,iter-1)) ge tol then begin
                ...
                nn=0
                Smax=[1.04967,1.06497,1.06684,1.06329,1.03844,0.991261,0.91655]
...
; store the outputs in a structure

;good = where(KD02_comb_Z + KD02_R23comb_Z + M91Z94C01_ave gt 0.0,ngood)
    good = name & ngood = n_elements(name)

    abundances = create_struct('id', 0L, 'Z_combined', 0.0, 'NII_OII', 0.0, $
      'R23_combined', 0.0, 'R23', 0.0, 'NII_SII', 0.0, 'S23', 0.0, $
      'C01', 0.0, 'M91', 0.0, 'Z94', 0.0, 'Comparison_average', 0.0, $
      'q', 0.0)
    abundances = replicate(abundances,ngood)

    abundances.id = name[good]
    abundances.Z_combined = KD02_comb_Z[good]
    abundances.NII_OII = KD02_NIIOII_Z[good]
    abundances.R23_combined = KD02_R23comb_Z[good]
    abundances.R23 = KD02_R23_Z[good]
    abundances.NII_SII = KD02_NIISII_Z[good]
    abundances.S23 = KD02_S23_Z[good]
    abundances.C01 = C01_Z[good]
    abundances.M91 = M91_Z[good]
    abundances.Z94 = Z94_Z[good]
    abundances.Comparison_average = M91Z94C01_ave[good]
    abundances.q = q_OIIIOII[good]
...
;printf,1,'KD02 = Kewley, L. J., & Dopita, M. A., 2002, ApJ, submitted '
