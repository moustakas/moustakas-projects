pro write_01kobulnicky, outdata
; jm04dec02uofa
...
; ---------------------------------------------------------------------------
    ...
    if (ngood ne 0L) then data[good].oii_3727 = 1D-16*transpose([ [raw[good].oii], [raw[good].oii_err] ])

    good = where(raw.oiii gt -900,ngood)
    if (ngood ne 0L) then begin
       data[good].oiii_5007 = 1D-16*transpose([ [raw[good].oiii], [raw[good].oiii_err] ])
       data[good].oiii_4959 = data[good].oiii_5007 / 3.0
    endif

    good = where(raw.ha gt -900,ngood)
    if (ngood ne 0L) then data[good].h_alpha = 1D-16*transpose([ [raw[good].ha], [raw[good].ha_err] ])

    good = where(raw.hb gt -900,ngood)
    if (ngood ne 0L) then data[good].h_beta = 1D-16*transpose([ [raw[good].hb], [raw[good].hb_err] ])
