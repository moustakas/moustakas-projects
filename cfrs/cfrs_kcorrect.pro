...
;   cfrs_kcorrect
; PURPOSE:
;   calculate K-corrections for standard CFRS input (from BRI to NUB)
; CALLING SEQUENCE:
;   kcorrect= cfrs_kcorrect(redshift [, nmgy=, ivar=, mag=, err=, $
;                           zcat=, /sdss, band_shift=, chi2=, rmaggies=, $
;                           omaggies=, vname=, oivar=, mass=, mtol=, $
;                           absmag=, amivar=, omega0=, omegal0= ])
; INPUTS:
;   redshift - [N] redshifts
;   zcat - [N] CFRS zcat-style structure, containing:
;                  .MAGB
;                  .MAGR
;                  .MAGI
;                  .SFD_EBV
...
;
;   Uses cfrs_to_maggies to convert zcat structure to Galactic
...
;-
;------------------------------------------------------------------------------
function cfrs_kcorrect, redshift, nmgy=nmgy, ivar=ivar, mag=mag, err=err, $
                        ...
                        filterlist=in_filterlist, silent=silent, intsfh=intsfh, vega=vega

common com_cfrs_kcorrect, out_rmatrix, out_zvals, band_shift, $
  cfrs_rmatrix, cfrs_zvals, vname
...
if(n_params() lt 1 OR $
  ...
  then begin
    doc_library, 'cfrs_kcorrect'
    return, -1
endif 

new_out_filterlist=['bessell_U.par',$
                    'bessell_B.par',$
                    'bessell_V.par',$
                    'bessell_R.par',$
                    'bessell_I.par',$
...
if(keyword_set(sdss)) then $
  new_out_filterlist=['sdss_u0.par',$
                      'sdss_g0.par',$
                      'sdss_r0.par',$
                      'sdss_i0.par',$
                      'sdss_z0.par']
...
cfrs_filterlist = ['bessell_'+['B','V','I'],'twomass_Ks']+'.par'
if(keyword_set(mag) or keyword_set(nmgy)) then begin
    mgy=fltarr(n_elements(cfrs_filterlist), n_elements(redshift))
    mgy_ivar=fltarr(n_elements(cfrs_filterlist), n_elements(redshift))
...
if(n_tags(zcat) gt 0) then $
  cfrs_to_maggies, zcat, mgy, mgy_ivar

;; call kcorrect
kcorrect, mgy, mgy_ivar, redshift, kcdum, band_shift=band_shift, $
  rmatrix=cfrs_rmatrix, zvals=cfrs_zvals, coeffs=coeffs, rmaggies=rmaggies, $
  vname=vname, mass=mass, mtol=mtol, absmag=absmag, amivar=amivar, $
  filterlist=cfrs_filterlist, silent=silent, intsfh=intsfh, chi2=chi2
...
if(keyword_set(closest)) then begin
    lambda_in=k_lambda_eff(filterlist=cfrs_filterlist)
...
; piece of code stolen from SDSS2BESSELL
