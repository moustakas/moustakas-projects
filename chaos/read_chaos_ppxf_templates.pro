function read_chaos_ppxf_templates, out_tempwave, velscale=velscale, $
...
; jm13apr01siena - read the output from BUILD_CHAOS_PPXF_TEMPLATES and
;   optionally resample to a constant ln-lambda; also optionally
;   convolve to the fiducial CHAOS instrumental velocity dispersion;
...
;   common ppxf_templates, tempflux, tempwave, temphdr, tempinfo
    ...
    if (n_elements(tempflux) eq 0) then begin
       ...
       templatefile = getenv('CHAOS_DATA')+'/bc03_'+metal+'.fits.gz'
...
; convolve to the CHAOS instrumental resolution
