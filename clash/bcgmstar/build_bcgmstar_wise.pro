...
pro build_bcgmstar_wise, query=query, parse=parse
; jm14jan01siena - match the BCGMSTAR sample to IRSA/WISE:
...
; search radius: 2"
    ...
    cat = read_bcgmstar_sample()
    ngal = n_elements(cat)

    if keyword_set(query) then begin
       wise_writeit, cat, path+'bcgmstar_wise_input.tbl'
       return
    endif
...
;         wisepath+'bcgmstar_'+dr+'_wise_input'+strtrim(ii+1,2)+'.tbl'

; code to parse the output and write the BCGMSTAR object_wise.fits file
    if keyword_set(parse) then begin
       tbl = im_read_tbl(path+'bcgmstar_wise_output.tbl')
...
;         tbl = im_read_tbl(wisepath+'bcgmstar_'+ver+'_wise_output'+strtrim(ii+1,2)+'.tbl')
...
;      endfor
       ...
       im_mwrfits, out, path+'bcgmstar_wise.fits', /clobber
    endif
