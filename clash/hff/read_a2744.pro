function read_a2744, photoz=photoz
; jm14jan10siena - read the photometric catalog
    ...
    cat = struct_addtags(cat,replicate({group: -1, galaxy: '', mu: -1.0},ngal))
...
; get rid of the tentative candidates    
    if keyword_set(photoz) eq 0 then cat = cat[where(cat.group ne 3)]
