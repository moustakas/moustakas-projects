...
; write out a Table for Adi
    print, 'Arc, mass (log Msun, demagnified), age (Myr), metallicity '+$
      '(with respect to solar=0.019), AV (mag), SFR (Msun/yr, demagnified), tau (Myr)'
...
; make a plot for the paper    
    ...
    readcol, 'lephare_combined_p_of_z.txt', lpzz, lppdf, /silent
...
;   ticks1 = loglevels(xrange1)
    ...
    im_legend, ['Arc 1.2','z_{phot} = 6.18^{+0.06}_{-0.03}'], /left, /top, box=0, margin=0
    oplot, model[this].wave, model[this].flux, line=0
    
; overplot the observed and model photometry
    used = where(cat[this].limit eq 0 and finite(cat[this].mag),nused)
    ...
    if (nupper ne 0) then begin
       oploterror, cat[this].weff[upper], cat[this].mag[upper], cat[this].magerr_up[upper], $
...
; P(z) inset
    ...
    plot, [0], [0], xsty=1, ysty=1, /nodata, /noerase, $
      yrange=[0,1.05], xrange=[zobj-1,6.8], position=pp, $
      ytitle='Probability', xtitle='Redshift', xtickinterval=0.5, $
...
;   psfile = 'z6arc_post.eps'
    ...
    plot, [0], [0], xsty=1, ysty=1, /noerase, /nodata, position=pos[*,0], yrange=[0,1.05], $
      xrange=[7.0,10.4], ytitle='Probability', charsize=1.4, $
    ...
    plot, [0], [0], xsty=5, ysty=5, /noerase, /nodata, yrange=[0,1.05], $
      xrange=[-0.02,0.92], position=pos[*,1], ytitle='', charsize=1.4, $
      xtitle=''
    ...
    plot, [0], [0], xsty=5, ysty=5, /noerase, /nodata, yrange=[0,1.1], $
      xrange=[-0.03,0.75], position=pos[*,3], ytitle='', $
    ...
    plot, [0], [0], xsty=1, ysty=1, /noerase, /nodata, yrange=[0,1.1], $
      xrange=[-0.03,0.75], position=pos[*,3], ytitle='Probability', $
      xtitle=textoidl('A_{V} (mag)'), $
      ytickname=replicate(' ',10), charsize=1.4, xtickinterval=0.2
    ...
    plot, [0], [0], xsty=5, ysty=5, /noerase, /nodata, yrange=[0,1.1], $
      xrange=[-0.05,1.1], position=pos[*,5], ytitle='', $
      xtitle=textoidl('\tau (Gyr)'), $
      ytickname=replicate(' ',10), charsize=1.4, xtickinterval=0.5
    im_plothist, tau[*,this], bin=0.08, /peak, /overplot, $
      /fill, fcolor=im_color('grey80')
...
; QAplot comparing the demagnified photometry of all the arcs
    psfile = datapath+'qaplot_allarcs_photometry.ps'
    ...
    xtitle1 = textoidl('Observed-Frame Wavelength (\AA)')
    xtitle2 = textoidl('Rest-Frame Wavelength (\AA)')
    ytitle1 = textoidl('m_{AB} (Demagnified)')
...
;   yrange = [32,20.0]
    ...
    for ii = 0, nobj-1 do begin
       ...
       plot, [0], [0], /nodata, noerase=(ii gt 0), xrange=xrange1, yrange=yrange, $
...
; Marc   
       used = where((maggies[*,ii] gt 0.0) and (ivarmaggies[*,ii] gt 0.0),nused)
       upper = where((maggies[*,ii] le 0.0) and (ivarmaggies[*,ii] gt 0.0),nupper)

       if (nused ne 0) then begin
          mab = maggies2mag(maggies[used,ii],ivar=ivarmaggies[used,ii],magerr=mab_err)
          oploterror, weff[used], mab+2.5*alog10(adi[ii].mu), mab_err, psym=symcat(16,thick=4), $
            symsize=2.0, color=im_color(color[ii]), $
            errcolor=im_color(color[ii]), errthick=!p.thick
       endif
       if (nupper ne 0) then begin
          mab = maggies2mag(1.0/sqrt(ivarmaggies[upper,ii]))
          djs_oplot, weff[upper], mab+2.5*alog10(adi[ii].mu), psym=symcat(11,thick=4), $
            symsize=2.0, color=im_color(color[ii])
       endif
    endfor
    im_legend, textoidl(galaxy), /left, /top, box=0, charsize=1.7, $
