pro megaspitzer_killerplot1, build_modelseds=build_modelseds, clobber=clobber
...
; --------------------------------------------------
; build the model SEDs
    if keyword_set(build_modelseds) then begin
...
;      sim.snr = 10.0^(randomu(seed,nmod)*(alog10(snrmax)-alog10(snrmin))+alog10(snrmin))
       ...
       sim.mgal = randomu(seed,nmod)*(maxmgal-minmgal)+minmgal
...
;         sfh1 = isedfit_reconstruct_sfh(sfh,/deb)
          ...
          flux1 = sim[ii].mgal*csp1*(pc10/dlum[ii])^2D/(1+sim[ii].z)

          sim[ii].mass = sim[ii].mgal*cspmstar
...
; the chosen SNR is preserved
          ...
          sim[ii].maggies = maggies1 + randomn(seed,nfilt)/sqrt(ivar)
