...
; echo "im_deep2_vmax" | idl > & im_deep2_vmax.log &

pro im_deep2_vmax, outstr, sdss=sdss, nocolor=nocolor, nooii=nooii, test=test
    ...
    if (not keyword_set(sdss)) then begin
       if keyword_set(test) then $
         alldeep = read_deep2(rows=lindgen(500)+15000L) else $
         alldeep = read_deep2()
;      if keyword_set(test) then $
...
;        alldeep = read_deep2_zcat(/good)
       ...
       kcorig = deep_kcorrect(alldeep.zhelio, zcat=alldeep, coeffs=allcoeffs, $
         absmag=allabsmag, /sdss, /silent)
...
;         interpol(reform(vmatrix#allcoeffs[*,ii]),restwave,3727.0) ; [erg/s/cm2/A]
       ...
       kcorig = deep_kcorrect(bri.z, mag=bri.bri, err=err, coeffs=coeffs, $
         absmag=absmag, /sdss)
       ngal=n_elements(bri)
       deep=bri
    endelse
...
;   zhi = [0.9,1.1,1.3,1.5,1.5,1.0,1.2,1.2]
    ...
    for iz = 0L, n_elements(zlo)-1L do begin
       ...
       zrange = where((alldeep.z gt zlo1) and (alldeep.z lt zhi1),ngal)
       deep = alldeep[zrange]
...
; now check which of the mock galaxies also pass the [O II] selection
          ...
          if (deep[igal].oii_3727_1_ew[1] eq -2.0) or (deep[igal].oii_3727_2_ew[1] eq -2.0) then begin
