...
pro alpha_x_echskysub, img, ivar, img_arc, objstr, ordr_str, img_new, CHK=chk, $
  ...
  use_objmask=use_objmask, nord=nord, evn=evn       ; jm09jan01nyu
...
;
 ...
 if (n_elements(use_objmask) ne 0L) then begin ; jm09jan01nyu

    mask_temp_obj = use_objmask 
...
; atv, maskimage, /bl
  ...
  for qq=ordr[0],ordr[1],ostep do begin
      ...
      ;; Order
      if (n_elements(nord) eq 0L) then nord = 4L ; jm09jan25nyu
...
;      slit_frac = 2.0 * slit_dist / slit_proj
      ...
      if nsky EQ 0 then begin
          print, 'x_echskysub:  No sky pixels!!  Continue at your own risk..'
          stop
          continue
      endif
      ...
      if (n_elements(evn) eq 0L) then $ ; jm09jan25nyu
...
;
      ...
      if (n_elements(sigrej) eq 0L) then sigrej = 5.0 ; jm09jan25nyu
      bset_prof = bspline_iterfit(wave_norm[sky], img_norm[sky],everyn=evn,$
                                  nord=nord, upper=sigrej, lower=sigrej, $
                                  INVVAR= img_norm_ivar[sky], /groupbadpix,$
                                  maxrej= 1, bkpt=bkpt_s, maxiter= 15L, $
                                  OUTMASK=pixmsk, yfit=yfit, /silent)

;    djs_plot, 10^wave_norm[sky], img_norm[sky], xsty=3
;    djs_oplot, 10^wave_norm[sky], yfit, color='red'
;    djs_oplot, 10^bkpt_s, replicate(0.0,n_elements(bkpt_s)), ps=4, sym=1.5, color='green'
...
;      endelse
      ...
      for qq=0L,nlin-1 do begin
          ...
          profile = profile*0.0+1.0 ; jm09jan01nyu
          ...
          mxs = max(sqrt(abs(sky_fit))) ; jm09jan01nyu
