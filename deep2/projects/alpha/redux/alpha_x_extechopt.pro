...
pro alpha_x_extechopt, img, skysub, ivar, ordr_str, obj_str, velpix, $
                 ...
                 EXTRACT_mask=extract_mask, redshift=redshift
...
; ---------------------------------------------------------------------------
; jm08apr16nyu; figure out which orders [OIII] 4959,5007 are in
; ---------------------------------------------------------------------------

  padwave = 0.0 ; 20.0
  these = where((img_arc*(ordermask gt 0.0) gt alog10(4959.0*(1.0+redshift)-padwave)) and $
    (img_arc*(ordermask gt 0.0) lt alog10(5007.0*(1.0+redshift)+padwave)),nthese)
  if (nthese eq 0L) then message, 'Problem here!'

  ordrs = ordermask[these[uniq(ordermask[these],sort(ordermask[these]))]]
  splog, 'Identified the [OIII] lines in orders ', ordrs
...
;         obj_wave = 10^bspline_valu(x_qckwav(objcen-slit_cen,ycol,$ ; jm08jul12nyu
;           ordr_shift[q].arc_m),dbl_wave_set)
;         obj_mask = 
...
; jm08apr15nyu          test!
;         plot, obj_str[q].box_wv/(1+redshift), obj_str[q].box_fx, $
;           charsize=2, xr=[5000,5015], ps=10
