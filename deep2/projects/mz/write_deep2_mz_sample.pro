pro write_deep2_mz_sample, kcorr, ispec, write=write
; jm08apr07nyu - written, based on WRITE_AGES_MZ_SAMPLE
    ...
    mzpath = deep2_path(/projects)+'mz/'
    ages_mzpath = ages_path(/projects)+'mz/'

    stime0 = systime(1)
    if keyword_set(write) then begin
       splogfile = mzpath+'write_deep2_mz_sample.log'
...
; is very close to the blue wavelength cut-off; in DEEP2_MZ_LOG12OH we
...
;   im_plothist, mz_kcorr.ugriz_absmag[2], bin=0.1, charsize=1.5
;   im_plothist, kcorr[parent[mz2]].ugriz_absmag[2], bin=0.1, /overplot, line=2, thick=2
;
;   im_plothist, mz_kcorr.ugriz_absmag[1]-mz_kcorr.ugriz_absmag[2], $
;     bin=0.05, charsize=1.5, xr=[-0.2,1.3]
;   im_plothist, kcorr[parent[mz2]].ugriz_absmag[1]-kcorr[parent[mz2]].ugriz_absmag[2], $
...
; ---------------------------------------------------------------------------
    ...
    loglb = alog10(10.0^(-0.4*(mz_kcorr.ubvri_absmag[1]-absmsun_b)))
    
    dlum = dluminosity(mz_kcorr.z,/cm)
    hb_lum_factor = mz_kcorr.cflux_4861*(4.0*!dpi*dlum^2.0)/lsun
    oii_lum_factor = mz_kcorr.cflux_3727*(4.0*!dpi*dlum^2.0)/lsun
...
; calibrated for 0.15<^{0.1}(g-r)<1.0
    ...
    gr = mz_kcorr.ugriz_absmag[1]-mz_kcorr.ugriz_absmag[2]
...
;   print
;
;   mzhii_kcorr = mz_kcorr[hii]
;   mzhiiplus_kcorr = mz_kcorr[hiiplus]
;   mzagn_kcorr = mz_kcorr[agn]
...
; rough oxygen abundances
    
;   if (n_elements(abund) eq 0L) or (n_elements(ohdust) eq 0L) then begin
;      temp = parent_ispec
;      these = where(temp.oiii_5007_ew[1] gt 0.0) & temp[these].oiii_4959_ew = temp[these].oiii_5007_ew*0.335
;      abund = mz_abundance(temp,nmonte=500L,snrcut_abundance=0.0,ewalpha=parent_ispec.ewalpha,/justew)
;      ohdust = im_assign_r23branch(abund,branchmethod=5L,r23branch=parent_ispec.r23branch,/kk04,debug=0)
;   endif
;
;   ohgood = where(ohdust.zstrong_ew_12oh_kk04 gt -900.0)
;   oh = ohdust.zstrong_ew_12oh_kk04
;   oherr = ohdust.zstrong_ew_12oh_kk04_err
...
; now make the plot    
    ...
    if keyword_set(write) then begin
       psname = mzpath+'deep2_mz_sample_selection.ps'
...
; k-corrections
       ...
       parentfile_kcorr = mzpath+'deep2_parent_kcorr.fits'
       splog, 'Writing '+parentfile_kcorr
       mwrfits, parent_kcorr, parentfile_kcorr, /create

       mzfile_kcorr = mzpath+'deep2_mz_kcorr.fits'
       splog, 'Writing '+mzfile_kcorr
       mwrfits, mz_kcorr, mzfile_kcorr, /create

; emission-line data       

       mzfile_ispec = mzpath+'deep2_mz_ispec.fits'
...
; finally compute and write out the oxygen abundances
       ...
       mz_log12oh, mz_ispec, nmonte=500L, snrcut=0.0, $
         logfile=mzpath+'mz_log12oh.deep2.log', $
         outfile_root=mzpath+'deep2_mz_log12oh', /write
