...
;------------------------------------------------------------------------------
function zfind_tinker, objflux, objivar, hdr=hdr, $
 ...
 nfind=nfind, width=width, zans_fixed=zans_fixed, _EXTRA=EXTRA, xwave=xwave
   ...
   if(keyword_set(xwave)) then begin
      objloglam0 = xwave[0]
      objdloglam = xwave[1] - xwave[0]
      ;print, "INPUT: ", objloglam0, objdloglam
   endif else begin
   ...
   if (keyword_set(eigenfile)) then begin
      ...
      if n_elements(starflux) LE 1 then begin
        ...
        ;print, "EIGEN INPUT: ", starloglam0, stardloglam0
      endelse
   endif
   ...
   ;; JLT
   ;---------
                                ; If the starflux is on a different
                                ; loglam scale, interpolate it on to
                                ; the object
   if(abs(stardloglam0-objdloglam)/objdloglam > 1.0E-2) then begin
      ;help, starflux
      ii = size(starflux)
      n_new = fix(ii[1]*stardloglam0/objdloglam)
      ;print, ii
      ;print, "n_new: ", n_new
      loglam = findgen(n_new)*objdloglam + starloglam0
      loglam0 = findgen(ii[1])*stardloglam0 + starloglam0
      xx = findgen(n_new, ii[2])*0

      for i=0L, ii[2]-1 do begin
         ;xx[*,i] = spline(loglam0, starflux[*,i], loglam)
         xx[*,i] = interpol(starflux[*,i],loglam0, loglam,/quadratic)
         w = wherenan(xx[*,i], n)
         if (n gt 0) then xx[w,i] = 0.5*(xx[w-1,i] + xx[w+1,i])
         ;xx[w,i] = 0
         ;stop
      endfor
      ;stop
      stardloglam0 = objdloglam
      starflux = xx
      ;help, starflux
      ;stop
   endif
