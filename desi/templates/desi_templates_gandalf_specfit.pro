...
pro desi_read_and_rebin, spec1d, objwave=objwave, $
...
; read the spectrum
    linearflux = spec1d/fluxscale ; note!
    linearferr = linearflux/thissnr
    linearflux = linearflux+randomn(seed,n_elements(linearferr))*linearferr
    linearwave = objwave
...
function desi_fit_lines, wave, flux, ferr, continuum, smooth_continuum, $
...
function desi_smooth_residuals, wave, flux, continuum, $
...
pro desi_zabs_vdisp, wave, flux, ferr, tempwave=tempwave, $
...
function desi_fit_continuum, wave, flux, ferr, tempwave=tempwave, $
...
pro desi_templates_gandalf_specfit, elgs=elgs, debug=debug, solar=solar

    if keyword_set(elgs) then begin
       prefix = 'elgs'
       tempversion = desi_elg_templates_version()
    endif else begin
       prefix = 'bgs'
       tempversion = desi_bgs_templates_version()
    endelse

; path names and emission-line file names
    version = 'v1.0' ; atlas_version(/ppxf_specfit)
    spec1dpath = getenv('DESI_ROOT')+'/spectro/templates/'+prefix+'_templates/'+tempversion+'/'
    specfitpath = getenv('DESI_ROOT')+'/ppxf/'
    alllinefile = specfitpath+'gandalf_elinelist_all.dat'
    linefile = specfitpath+'gandalf_elinelist_'+version+'.dat'

    temp = mrdfits(spec1dpath+prefix+'_templates_obs_'+tempversion+'.fits',0,hdr)
    data = mrdfits(spec1dpath+prefix+'_templates_obs_'+tempversion+'.fits',1)
    objwave = make_wave(hdr)
    keep = where(objwave gt 3650.0 and objwave lt 6000.0,npix)
;   keep = where(objwave gt 3650.0 and objwave lt 9500.0,npix)
    temp = temp[keep,*]
    objwave = objwave[keep]
...
; convolve to ATLAS pixel size and instrumental resolution
    velscale = desi_templates_ppxf_velscale()
    inst_vdisp = 0D ; desi_templates_instvdisp()
    line_inst_vdisp = 0.0 ; inst_vdisp/3.0 ; [km/s]
    if (n_elements(fluxscale) eq 0) then fluxscale = 1D-17

    tempflux = read_desi_templates_ppxf(tempwave,ntemp=ntemp,$
...
; get the relevant list of spectra and specify the output file name 
    ...
    specdatafile = specfitpath+prefix+'_specdata_raw_'+version+'.fits'
...
; read and logarithmically rebin the spectrum
       desi_read_and_rebin, temp[*,iobj], objwave=objwave, $
         velscale=velscale, fluxscale=fluxscale, rebin_wave=wave, $
...
; get the absorption-line redshift and velocity dispersion
          desi_zabs_vdisp, wave, flux, ferr, tempwave=tempwave, $
            tempflux=tempflux1, velscale=velscale, zabs_guess=data[iobj].z,$
...
; a better estimate of the absorption-line redshift
          continuum1 = desi_fit_continuum(wave,flux,ferr,tempwave=tempwave, $
...
; find the metallicity that has the minimum chi^2
       ...
       smooth_continuum = continuum*0
;      smooth_continuum = desi_smooth_residuals(wave,flux,$
...
; finally fit the emission lines
       sol = desi_fit_lines(wave,flux,ferr,continuum,smooth_continuum,$
         zabs=zabs,vdisp=vdisp,velscale=velscale,linefile=linefile,$
