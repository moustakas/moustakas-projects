function read_desi_templates_ppxf, out_tempwave, velscale=velscale, $
...
; jm10feb25ucsd - read the output from BUILD_ATLAS_PPXF_TEMPLATES and
...
;   common ppxf_templates, tempflux, tempwave, temphdr, tempinfo
    ...
    if (n_elements(tempflux) eq 0) then begin
       if keyword_set(solar) then metal = 'Z0.0190' else $
         metal = 'Z'+['0.0031','0.0190','0.0300']
       templatefile = getenv('DESI_ROOT')+'/ppxf/fsps_'+metal+'_'+version+'.fits.gz'
       for ii = 0, n_elements(templatefile)-1 do begin
          splog, 'Reading '+templatefile[ii]
          tempflux1 = mrdfits(templatefile[ii],0,temphdr)
          tempinfo1 = mrdfits(templatefile[ii],1)
