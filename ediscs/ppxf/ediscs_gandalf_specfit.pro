;+
; NAME:
;   EDISCS_GANDALF_SPECFIT
;
; PURPOSE:
;   Fit the EDISCS spectra using PPXF/GANDALF.
;
; INPUTS: 
;
; KEYWORD PARAMETERS: 
...
pro ediscs_read_and_rebin, specfile, spec1dpath=spec1dpath, $
  velscale=velscale, fluxscale=fluxscale, rebin_wave=wave, $
  rebin_flux=flux, rebin_ferr=ferr, snr=snr
; internal routine to read and logarithmically rebin the EDISCS
; spectra  
...
function ediscs_fit_lines, wave, flux, ferr, continuum, smooth_continuum, $
...
; mask non emission-line pixels and other bad pixels
    goodpixels = gandalf_mask_emission_lines(npix,vsys,$
      alllinepars,velscale,wave[0],velscale/light,$
      sigma=sigma_smooth)
    goodlinepixels = lindgen(npix)
    goodlinepixels = cmset_op(lindgen(npix),'and',/not2,goodpixels)
    badpixels = where((restferr gt 1E5) or (abs(restflux) gt 100.0),nbad)
    if (nbad ne 0L) then goodlinepixels = cmset_op(goodlinepixels,'and',/not2,badpixels)
...
function ediscs_smooth_residuals, wave, flux, continuum, $
...
pro ediscs_zabs_vdisp, wave, flux, ferr, tempwave=tempwave, $
...
; if the velocity dispersion is not well-measured then put a floor
    vdisp_snr = vdisp/(vdisp_err+(vdisp_err eq 0))*(vdisp_err ne 0)
...
function ediscs_fit_continuum, wave, flux, ferr, tempwave=tempwave, $
  tempflux=tempflux, velscale=velscale, zabs_guess=zabs_guess, $
  vdisp_guess=vdisp_guess, ebv_guess=ebv_guess, alllinefile=alllinefile, $
  goodpixels=goodpixels, final_zabs=zabs, final_vdisp=vdisp, $
  final_ebv=ebv, err_final_zabs=zabs_err, err_final_vdisp=vdisp_err, $
  chi2=chi2, continuum_coeff=continuum_coeff, debug=debug
; internal support routine to fit the continuum
...
; mask out bad pixels and pixels affected by emission lines
    linepars = read_gandalf_elinelist(alllinefile)
    goodpixels = gandalf_mask_emission_lines(npix,vsys*0.0,$
      linepars,velscale,wave[0],velscale/light,$
      sigma=500.0)
    badpixels = where((restferr gt 1E5) or (abs(restflux) gt 100.0),nbad)
    if (nbad ne 0L) then goodpixels = cmset_op(goodpixels,'and',/not2,badpixels)

    ebv = ebv_guess
    im_ppxf, fit_tempflux, restflux, restferr, velscale, start, $
      sol, goodpixels=goodpixels, plot=doplot, moments=2, degree=-1, $
...
;function ediscs_fit_continuum_final, wave, flux, ferr, tempwave=tempwave, $
...
pro ediscs_gandalf_specfit, debug=debug, broad=broad, $
  run34=run34, solar=solar
; the pixel size in RUN34 was 1.6 A/pix vs 1.2 A/pix in RUN12 (see
;   Halliday+04), so we have to fit the spectra separately
...
; path names and emission-line file names
    ...
    alllinefile = ediscs_path(/ppxf)+'gandalf_elinelist_all.dat'
...
; read the spectroscopic info file    
    ediscs = read_ediscs(/spec1d)
    if keyword_set(run34) then begin
       these = where(ediscs.dwave gt 1.5)
       thisrun = 'run34'
    endif else begin
       these = where(ediscs.dwave lt 1.5)
       thisrun = 'run12'
    endelse
    ediscs = ediscs[these]
    
; read the templates (see BUILD_EDISCS_PPXF_TEMPLATES); resample and
; convolve to EDISCS pixel size and instrumental resolution
    velscale = ediscs_ppxf_velscale(run34=run34)
    inst_vdisp = ediscs_ppxf_instvdisp(run34=run34)
    line_inst_vdisp = 0.0 ; inst_vdisp/3.0 ; [km/s]
    if (n_elements(fluxscale) eq 0) then fluxscale = 1D-17

    tempflux = read_ediscs_ppxf_templates(tempwave,ntemp=ntemp,$
      velscale=velscale,inst_vdisp=inst_vdisp,tempinfo=info,$
      solar=solar)
    metal = info.metallicity
    nmetal = n_elements(metal)

; get the filenames and specify the output files
    speclist = strtrim(ediscs.specfile,2)
    ...
    specdatafile = specfitpath+'ediscs_specdata_raw_'+suffix+'_'+version+'.fits'
...
; fit each object using GANDALF/PPXF
    t0 = systime(1)
;   for iobj = 493, 494 do begin
    for iobj = 0, nobj-1 do begin
       splog, file_basename(speclist[iobj])+': '+string(iobj,$
         format='(I4.4)')+'/'+string(nobj-1,format='(I4.4)')

; read and logarithmically rebin the spectrum
       ediscs_read_and_rebin, speclist[iobj], spec1dpath=spec1dpath, $
         velscale=velscale, fluxscale=fluxscale, rebin_wave=wave, $
         rebin_flux=flux, rebin_ferr=ferr, snr=continuum_snr

; loop on each metallicity       
       for imetal = 0, nmetal-1 do begin
;         splog, 'Z = '+strtrim(metal[imetal])
          tempflux1 = tempflux[*,*,imetal]

; fit the continuum          
          continuum1 = ediscs_fit_continuum(wave,flux,ferr,tempwave=tempwave, $
            tempflux=tempflux1,velscale=velscale,zabs_guess=ediscs[iobj].z,$
            vdisp_guess=vdisp_guess,ebv_guess=ebv_guess,alllinefile=alllinefile,$
            final_zabs=zabs1,final_vdisp=vdisp1,final_ebv=ebv1,$
            err_final_zabs=zabs_err1, err_final_vdisp=vdisp_err1,chi2=chi21,$
            continuum_coeff=coeff1,debug=debug)

;; ---------------
;; get the absorption-line redshift and velocity dispersion
;          ediscs_zabs_vdisp, wave, flux, ferr, tempwave=tempwave, $
;            tempflux=tempflux1, velscale=velscale, zabs_guess=ediscs[iobj].z,$
...
;            debug=debug
;
;; now refit the continuum, fixing ZABS and VDISP
;          continuum1 = ediscs_fit_continuum_final(wave,flux,ferr,tempwave=tempwave, $
...
; smooth the residuals before before fitting the lines
       smooth_continuum = ediscs_smooth_residuals(wave,flux,$
         continuum,zabs=zabs,velscale=velscale,broad=broad,$
         linefile=linefile,debug=debug)

; finally fit the emission lines
       sol = ediscs_fit_lines(wave,flux,ferr,continuum,smooth_continuum,$
         zabs=zabs,vdisp=vdisp,velscale=velscale,linefile=linefile,$
...
; pack everything into a structure          
       ...
       specdata1.ediscs_id = ediscs[iobj].ediscs_id
       specdata1.galaxy = strtrim(ediscs[iobj].galaxy,2)
       specdata1.specfile = speclist[iobj]
       specdata1.cluster = strtrim(ediscs[iobj].cluster,2)
       specdata1.ra = ediscs[iobj].ra
       specdata1.dec = ediscs[iobj].dec
       ...
       specdata1.z = ediscs[iobj].z ; from NED
       specdata1.zabs = zabs
