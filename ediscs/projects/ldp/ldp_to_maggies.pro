pro ldp_to_maggies, cat, maggies, ivarmaggies, filterlist=filterlist
; jm16may18siena - convert an LDP-style catalog to maggies

    ngal = n_elements(cat)    
    if (ngal le 0L) then begin
       doc_library, 'ldp_to_maggies'
       return
    endif

    filterlist = ldp_filterlist(short=short)
