...
;
; PURPOSE:
;      Parse the "raw" FITS files from the 2003 observations into 
;      iSPEC-compatible files.
;
; CALLING SEQUENCE:
;      unpack_2003_raw, /wfits
;
; INPUTS:
;      None.
;
...
;
; KEYWORD PARAMETERS:
;      wfits - write out FITS files
;
; OUTPUTS:
;      FITS files with new file names and updated headers are
;      written. 
;
; OPTIONAL OUTPUTS:
;      None.
;
; PROCEDURES USED:
;      HEADFITS(), ESO_HEADER_FORAGE(), STRUCT_TRIMTAGS(), SPLOG,
;      UPDATE_ESO_2003_HEADER(), DJS_MODFITS, MRDFITS(), SXADDPAR,
;      MWRFITS, DJS_AVSIGCLIP()
;
; COMMENTS:
;      There is a lot of un-neccesary data in the RAW subdirectory.
;      we will not need chip2 images (no data), nor calibration data
;      that are not part of our science program (see DELETE_CHIP2).
;
;      This routine distinguishes between MOS and LSS observations. 
;
; EXAMPLES:
;
...
pro unpack_2003_raw, wfits=wfits

    rootpath = '/d0/ioannis/ediscs/2003/'
...
; ---------------------------------------------------------------------------
; copy the bias frame for CHIP1 to the output directory    
; ---------------------------------------------------------------------------

    splog, 'Updating bias file header [bias_chip1.fits].'
    if (file_test(cwdpath+'bias_chip1.fits',/regular) eq 0L) then begin
       splog, 'Bias file '+cwdpath+'bias_chip1.fits not found.'
       return
    endif

    h = headfits(cwdpath+'bias_chip1.fits')
    hinfo = eso_header_forage(h)
    
    newh = update_eso_2003_header(h,hinfo,/bias)
    
    if keyword_set(wfits) then begin

       splog, 'Writing '+outpath+'a.bias_chip1.fits'+'.'
       spawn, ['/bin/cp -f '+cwdpath+'bias_chip1.fits '+outpath+'a.bias_chip1.fits'], /sh
       djs_modfits, outpath+'a.bias_chip1.fits', 0, newh
...
;      lss_arc_avg = djs_avsigclip(lss_arc,3,sigrej=3.0,outmask=lss_arcmask)
       ...
       newh = update_eso_2003_header(h,lss_arcinfo[0],/arclamp)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       newh = update_eso_2003_header(h,lss_flatinfo[0],/domeflat)
...
;      mos_arc_left_avg = djs_avsigclip(mos_arc_left,3,sigrej=3.0,outmask=mos_arc_leftmask)
       ...
       newh = update_eso_2003_header(h,mos_arcinfo[indx_mos_arc_left[0]],/arclamp,/left)
...
;      mos_arc_center_avg = djs_avsigclip(mos_arc_center,3,sigrej=3.0,outmask=mos_arc_centermask)
       ...
       newh = update_eso_2003_header(h,mos_arcinfo[indx_mos_arc_center[0]],/arclamp,/center)
...
;      mos_arc_right_avg = djs_avsigclip(mos_arc_right,3,sigrej=3.0,outmask=mos_arc_rightmask)
       ...
       newh = update_eso_2003_header(h,mos_arcinfo[indx_mos_arc_right[0]],/arclamp,/right)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       newh = update_eso_2003_header(h,mos_flatinfo[indx_mos_flat_left[0]],/domeflat,/left)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       newh = update_eso_2003_header(h,mos_flatinfo[indx_mos_flat_center[0]],/domeflat,/center)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       newh = update_eso_2003_header(h,mos_flatinfo[indx_mos_flat_right[0]],/domeflat,/right)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       for i = 0L, nlss_obj-1L do begin
          ...
          newh = update_eso_2003_header(h,lss_objinfo[i],/object)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       for i = 0L, nmos_obj_left-1L do begin
          ...
          newh = update_eso_2003_header(h,mos_objinfo[indx_mos_obj_left[i]],/object,/left)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       for i = 0L, nmos_obj_center-1L do begin
          ...
          newh = update_eso_2003_header(h,mos_objinfo[indx_mos_obj_center[i]],/object,/center)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       for i = 0L, nmos_obj_right-1L do begin
          ...
          newh = update_eso_2003_header(h,mos_objinfo[indx_mos_obj_right[i]],/object,/right)
