...
pro unpack_2004_raw, wfits=wfits
...
; distinguish between MOS and LSS observations    
    ...
    rootpath = '/d1/ioannis/ediscs/2004/'
...
; ---------------------------------------------------------------------------
    ...
    newh = update_eso_2004_header(h,hinfo,/bias)
...
;      lss_arc_avg = djs_avsigclip(lss_arc,3,sigrej=3.0,outmask=lss_arcmask)
       ...
       newh = update_eso_2004_header(h,lss_arcinfo[0],/arclamp)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       newh = update_eso_2004_header(h,lss_flatinfo[0],/domeflat)
...
;      mos_arc_left_avg = djs_avsigclip(mos_arc_left,3,sigrej=3.0,outmask=mos_arc_leftmask)
       ...
       newh = update_eso_2004_header(h,mos_arcinfo[indx_mos_arc_left[0]],/arclamp,/left)
...
;      mos_arc_center_avg = djs_avsigclip(mos_arc_center,3,sigrej=3.0,outmask=mos_arc_centermask)
       ...
       newh = update_eso_2004_header(h,mos_arcinfo[indx_mos_arc_center[0]],/arclamp,/center)
...
;      mos_arc_right_avg = djs_avsigclip(mos_arc_right,3,sigrej=3.0,outmask=mos_arc_rightmask)
       ...
       newh = update_eso_2004_header(h,mos_arcinfo[indx_mos_arc_right[0]],/arclamp,/right)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       newh = update_eso_2004_header(h,mos_flatinfo[indx_mos_flat_left[0]],/domeflat,/left)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       newh = update_eso_2004_header(h,mos_flatinfo[indx_mos_flat_center[0]],/domeflat,/center)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       newh = update_eso_2004_header(h,mos_flatinfo[indx_mos_flat_right[0]],/domeflat,/right)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       for i = 0L, nlss_obj-1L do begin
          ...
          newh = update_eso_2004_header(h,lss_objinfo[i],/object)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       for i = 0L, nmos_obj_left-1L do begin
          ...
          newh = update_eso_2004_header(h,mos_objinfo[indx_mos_obj_left[i]],/object,/left)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       for i = 0L, nmos_obj_center-1L do begin
          ...
          newh = update_eso_2004_header(h,mos_objinfo[indx_mos_obj_center[i]],/object,/center)
...
; ---------------------------------------------------------------------------
    ...
    if keyword_set(wfits) then begin
       ...
       for i = 0L, nmos_obj_right-1L do begin
          ...
          newh = update_eso_2004_header(h,mos_objinfo[indx_mos_obj_right[i]],/object,/right)
