...
;   gdds_kcorrect
; PURPOSE:
;   calculate K-corrections for standard GDDS input (from BRI to NUB)
; CALLING SEQUENCE:
;   kcorrect= gdds_kcorrect(redshift [, nmgy=, ivar=, mag=, err=, $
;                           zcat=, /sdss, band_shift=, chi2=, rmaggies=, $
;                           omaggies=, vname=, oivar=, mass=, mtol=, $
;                           absmag=, amivar=, omega0=, omegal0= ])
; INPUTS:
;   redshift - [N] redshifts
;   zcat - [N] GDDS zcat-style structure, containing:
;                  .MAGB
;                  .MAGR
;                  .MAGI
;                  .SFD_EBV
...
;
;   Uses gdds_to_maggies to convert zcat structure to Galactic
...
;-
;------------------------------------------------------------------------------
function gdds_kcorrect, redshift, nmgy=nmgy, ivar=ivar, mag=mag, err=err, $
...
common com_gdds_kcorrect, out_rmatrix, out_zvals, band_shift, $
  gdds_rmatrix, gdds_zvals, vname
...
gdds_filterlist = 'gdds_'+['B','V','R','I','z','H','K']+'.par'
if(keyword_set(mag) or keyword_set(nmgy)) then begin
    mgy=fltarr(n_elements(gdds_filterlist), n_elements(redshift))
    mgy_ivar=fltarr(n_elements(gdds_filterlist), n_elements(redshift))
...
if(n_tags(zcat) gt 0) then $
  gdds_to_maggies, zcat, mgy, mgy_ivar

;; call kcorrect
kcorrect, mgy, mgy_ivar, redshift, kcdum, band_shift=band_shift, $
  rmatrix=gdds_rmatrix, zvals=gdds_zvals, coeffs=coeffs, rmaggies=rmaggies, $
  vname=vname, mass=mass, mtol=mtol, absmag=absmag, amivar=amivar, $
  filterlist=gdds_filterlist, silent=silent, intsfh=intsfh, chi2=chi2
...
if(keyword_set(closest)) then begin
    lambda_in=k_lambda_eff(filterlist=gdds_filterlist)
