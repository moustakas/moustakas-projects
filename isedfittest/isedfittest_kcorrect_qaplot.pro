...
pro test_isedfit_kcorrect_qaplot, info, psfile=psfile, $
  in_filterlist=in_filterlist, clobber=clobber
    
    ngal = n_elements(info)
    if (ngal eq 0L) then begin
       doc_library, 'test_isedfit_kcorrect_qaplot'
       return
    endif

; restore the filters and best-fitting models
    in_filterlist = sdss_filterlist()
