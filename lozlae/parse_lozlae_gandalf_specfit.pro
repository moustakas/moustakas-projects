pro parse_lozlae_gandalf_specfit, solar=solar, debug=debug
; jm10dec06ucsd - parse the output from lozlae_gandalf_specfit
...
; path names and emission-line file name
    ...
    indexfile = specfitpath+'lick_indexlist_v2.0.dat'

    if keyword_set(solar) then suffix = '_solar' else suffix = '_subsolar'
    rawfiles = specfitpath+'lozlae_specdata_raw'+suffix+'.fits.gz'
...
; luminosity-weighted age for each object
    junk = read_lozlae_ppxf_templates(tempinfo=tempinfo,solar=solar)
