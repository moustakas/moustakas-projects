pro render_lozlae_gandalf_lineplot, wave, flux_noc, linefit, specdata, $
...
;     max(linefit[xx[0]:xx[1]]))*[-0.12,1.2]
    ...
    oplot_lozlae_gandalf_linewave, specdata
    im_legend, linelabel, /left, /top, box=0, charsize=1.2

return
end

pro oplot_lozlae_gandalf_linewave, specdata
...
pro qaplot_lozlae_gandalf, specdata, specfit, pos1, pos2, $
  linear=linear
...
; define some convenient internal variables
    galaxy = 'Lozlae '+string(specdata.id,format='(I2.2)')
...
;   xrange[*,0] = 3426.0
    ...
    for ii = 0, npanel-1 do begin
       render_lozlae_gandalf_lineplot, wave, flux_noc, linefit, $
...
pro qaplot_lozlae_gandalf_specfit, specdata, specfit, $
  linear=linear, solar=solar, psfile=psfile
; jm09nov30ucsd - build a QAplot from the READ_LOZLAE_GANDALF_SPECFIT
;   output; NOTE! /LINEAR must be set if it was set when calling
;   READ_LOZLAE_GANDALF_SPECFIT(), otherwise DOOM!! 
;
; alldata = read_lozlae_gandalf(/ppxf)
; data = alldata[where(alldata.z gt 0.5 and alldata.z lt 0.5003)]
; specfit = read_lozlae_gandalf_specfit(data,/linear)
; qaplot_lozlae_gandalf_specfit, data, specfit, /linear

    nobj = n_elements(specdata)
    if (nobj eq 0L) then begin
       doc_library, 'qaplot_lozlae_gandalf_specfit'
       return
    endif

    specfit = read_lozlae_gandalf_specfit(specdata,linear=linear,solar=solar)
    
    if keyword_set(solar) then suffix = '_solar' else suffix = '_subsolar'
    if (n_elements(psfile) eq 0) then psfile = $
      'qaplot_lozlae'+suffix+'.ps'
    ...
    for jj = 0, nobj-1 do begin
       print, format='("Object ",I0,"/",I0,A10,$)', jj, nobj-1, $
         string(13b)
       qaplot_lozlae_gandalf, specdata[jj], specfit[jj], $
