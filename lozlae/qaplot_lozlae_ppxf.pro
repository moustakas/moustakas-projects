...
; define some convenient internal variables
    galaxy = 'EGS'+string(specdata.id,format='(I0)')
...
; plot the data       
       djs_oplot, wave, scale*flux, ps=10, color=fsc_color('grey',101), thick=8
       djs_oplot, wave, scale*(continuum+linefit), ps=10, thick=6, color=fsc_color('firebrick',101)
...
pro qaplot_lozlae_ppxf, ps=ps
; jm10dec09ucsd - example PPXF fit for the paper

    pspath = lozlae_path()

    if (keyword_set(ps) eq 0) then indx = 6
    specdata = mrdfits(pspath+'lozlae_specdata_subsolar.fits.gz',1,row=indx)
    specfit = read_lozlae_gandalf_specfit(specdata)
    nobj = n_elements(specdata)
    
; set up the plot
    if keyword_set(ps) then suffix = '_all.ps' else $
      suffix = '_EGS'+strtrim(specdata.id,2)+'.eps'
    psfile = pspath+'qaplot_ppxf'+suffix
    ...
    im_plotconfig, psfile=psfile, /psclose, gzip=keyword_set(ps)
