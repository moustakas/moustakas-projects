function read_lozlae_ppxf_templates, out_tempwave, velscale=velscale, $
...
; jm10feb25ucsd - read the output from BUILD_LOZLAE_PPXF_TEMPLATES and
;   optionally resample to a constant ln-lambda; also optionally
;   convolve to the fiducial SINGS instrumental velocity dispersion;
...
;        metal = ['0.004','0.020','0.050']
       templatefile = lozlae_path()+'bc03_'+metal+'.fits.gz'
    ...
    ntemppix = tempsize[0]
    ntemp = tempsize[1]
    if (ndim eq 2) then nmetal = 1 else nmetal = tempsize[2]
...
; convolve to the SINGS instrumental resolution
