...
;       spectral nfgs.  Use the best-fitting Bruzual & Charlot models
...
pro nfgs_synthmags, nfgsinfo, nfgs, outfile=outfile, doplot=doplot, write=write

    if n_elements(outfile) eq 0L then outfile = 'nfgs_int_synthmags.fits'
    
    datapath = nfgs_path(/spec1d)
    outpath = nfgs_path(/specfit)

; read the structures and sort by RA    
    
    if (n_elements(nfgsinfo) eq 0L) then nfgsinfo = nfgs_read_info()
    if (n_elements(nfgs) eq 0L) then nfgs = read_nfgs(/silent)

    nfgsinfo = nfgsinfo[sort(nfgsinfo.ra)]
    nfgs = nfgs[sort(nfgs.ra)]
    nnfgs = n_elements(nfgs)
    nnfgsinfo = n_elements(nfgsinfo)

; only synthesize magnitudes for objects that have been fitted

    skipindx = cmset_op(nfgsinfo.nfgs_id,'AND',/not2,nfgs.nfgs_id,/index)
    fitindx = cmset_op(nfgsinfo.nfgs_id,'AND',nfgs.nfgs_id,/index)
...
; store it in the ISPECLINEFIT(); redshift the best-fitting spectrum
       ...
       testflux = mrdfits(datapath+strtrim(nfgs[i].drift_file,2),0,/silent)
       ferr = mrdfits(datapath+strtrim(nfgs[i].drift_file,2),1,/silent)
...
;      spec = rd1dspec(nfgs[i].driftfile,/silent,datapath=datapath)
;      flux = spec.spec
;      ferr = spec.sigspec
;      refwave = spec.wave
...
; copy "result" into "bigresult"
    ...
    junk = bigresult[fitindx]
    copy_struct, result, junk
    bigresult[fitindx] = junk
