...
;
; PURPOSE:
;       Flux-calibrate the sc1120 spectra.
;
; CALLING SEQUENCE:
;
;
...
;       J. Moustakas, 2005 Jan 17, U of A - written
;-

pro sc1120_calibrate, speclist, sensname, prefix=prefix, datapath=datapath, $
...
;      print, min(wave), min(senswave), max(wave), max(senswave)
       
;      good = where((wave ge min(senswave)) and (wave le max(senswave)))
;      flux = flux[good]
;      ferr = ferr[good]
;      wave = wave[good]
