;+
; NAME:
;   PARSE_SINGS_GANDALF_SPECFIT
;
; PURPOSE:
;   Parse the output from SINGS_GANDALF_SPECFIT and write out the final
...
pro parse_sings_gandalf_specfit, nuclear=nuclear, drift20=drift20, $
  drift56=drift56, debug=debug, solar=solar
...
; path names and emission-line file names
    ...
    indexfile = sings_path(/ppxf)+'lick_indexlist_'+version+'.dat'
    linefile = sings_path(/ppxf)+'gandalf_elinelist_'+version+'.dat'
    linefile_broad = repstr(linefile,'elinelist_','elinelist_broad_')

; parse the full list of files written by SINGS_GANDALF_SPECFIT, or a
; subset
    ...
    rawfile = specfitpath+'sings_specdata_raw_'+suffix+'_'+version+'.fits.gz'
...
; luminosity-weighted age for each object
    junk = read_sings_ppxf_templates(tempinfo=tempinfo,solar=solar)
