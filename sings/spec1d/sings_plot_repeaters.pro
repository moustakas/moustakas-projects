...
;
; CALLING SEQUENCE:
;       sings_plot_repeaters, /postscript
;
; INPUTS:
;
; OPTIONAL INPUTS:
...
;       generate an SINGS_COMBINE_REPEATERS.TXT file and run
...
pro sings_plot_repeaters, postscript=postscript
...
; initialize some plotting variables
    ...
    if keyword_set(postscript) then begin
       dfpsplot, analysis_path+'sings_plot_repeaters.ps', xsize=xpage, ysize=ypage, /color
       postthick = 8.0
       postthick2 = 5.0
    endif else begin
...
; read all the repeaters
    ...
    uindx = cmset_op(uniq(allgalaxy),'OR',uniq(allinfo.aperwid)) ; uniq(allgalaxy)
...
;         djs_oplot, finalwave, objflux[*,irepeat]/maxnorm-offset, $
       ...
       if (info[igalaxy].scanlen lt 3.0) then legstr = legstr+' Nuclear'
       if (info[igalaxy].scanlen eq 20.0) then legstr = legstr+' 20" Scan'
       if (info[igalaxy].scanlen gt 50.0) then legstr = legstr+' 56" Scan'
