...
;           if j eq ncut-1L then print, transpose(total(alog10(g[indx,*]),1))
;            if j eq ncut-1L then begin
;                print, [transpose(-nstars*alog10(total(g,1))),$
;                        transpose(total(alog10(g[indx,*]),1)), $
;                        transpose(lnlike[*,j])]
;                stop
;            endif
        endfor
...
pro testidl, objname, iter=iter, binsize=binsize, minmag=minmag, maxmag=maxmag, $
