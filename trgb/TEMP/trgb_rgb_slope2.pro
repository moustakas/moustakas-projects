...
pro trgb_rgb_slope2
	...
	cd, '/deepscr1/ioannis/trgb2/ugc07577'
...
;       window, 0, xs=800, ys=800
        ...
        !p.multi = 0;[0,1,3]
	temp=min(abs(xmag-23.2),wh)
        plot, xmag-22.95, lfunction/float(lfunction(wh)), ps=10, line=0, $;color=7, $
          xsty=3, ysty=3, /ylog, xr=[-.5,1.], xminor=2, $
 ;         tit='UGC 07577 Luminosity Function', $
          ytit = 'Normalized Number Density', xtit='Magnitude Relative to Tip'
...
; --------------------------------------------------------------------------------
        ...
        make_lfunction, imags, binsize, minmag, maxmag, lfunction, xmag
	temp=min(abs(xmag-22.),wh)
        oplot, xmag-21.8, lfunction/float(lfunction(wh)), ps=10, line=1
;, $; color=5, $
;          xsty=3, ysty=3, /ylog, xr=[21.3,22.8], xminor=2, $
...
; --------------------------------------------------------------------------------
	...
	print,info[indx].a_i 
        make_lfunction, imags, binsize, minmag, maxmag, lfunction, xmag
	temp=min(abs(xmag-18.15),wh)
        oplot, xmag-18., lfunction/float(lfunction(wh)), ps=10, line=2
;, $; color=5, $
;          xsty=3, ysty=3, /ylog, xr=[17.55,18.55], xminor=2, $
