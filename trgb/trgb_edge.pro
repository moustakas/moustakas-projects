...
;	modified and made user-friendly, jm00sep10uofa	
;
;-
...
; print up some useful information
        ...
        linsmooth = smooth2(float(lf.hist),4)		; smooth the histograms
        logsmooth = smooth2(alog10(float(lf.hist)>1.),4)

        noise = sqrt(2.*(linsmooth>1.))	; Poisson noise per magnitude bin

        resplin = shift(float(lf.hist),-1) - shift(float(lf.hist),1) 			   ; linear response
        resplog = shift(alog10(float(lf.hist)>1.),-1) - shift(alog10(float(lf.hist)>1.),1) ; log response
