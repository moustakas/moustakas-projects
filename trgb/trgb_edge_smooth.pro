function badbins, shist, hist, mag

	resid = shist-hist

        window, 10, xs=450, ys=450
        plotsym, 0, 1, /fill
        plot, mag, resid, ps=8, color=3, xsty=3, ysty=3

        plot, mag, shist, /ylog, color=3, xsty=3, ysty=3
        oplot, mag, hist, line=2, color=5
...
pro trgb_edge_smooth, objname, iter=iter, binsize=binsize, minmag=minmag, maxmag=maxmag, $
...
; trap errors
        ...
        goodlin = where(finite(resplin) eq 1L)
        goodlog = where(((finite(resplog) and finite(alog10(float(lf.hist)))) eq 1L) $
                        and alog10(float(lf.hist)) gt float(0))

; find the noise in each bin by Gaussian smoothing the LF

        linsmooth = smooth2(float(lf.hist[goodlin]),2)
        logsmooth = smooth2(alog10((float(lf.hist))[goodlog]),2)

;       noisebins = badbins(logsmooth,alog10((float(lf.hist))[goodlog]),lf.mag[goodlog])

; trap errors again

        goodlin2 = where(linsmooth gt float(0))
        goodlog2 = where(logsmooth gt float(0))

; Poisson noise goes as the sqrt(N)

        linnoise = sqrt(linsmooth[goodlin2])
        lognoise = sqrt(logsmooth[goodlog2])

; find the maximum response

        maxlin = max(resplin[goodlin[goodlin2]]/linnoise[goodlin[goodlin2]],linindx)
        trgblin = (lf.mag[goodlin[goodlin2]])[linindx]

        maxlog = max(resplog[goodlog[goodlog2]]/lognoise[goodlog[goodlog2]],logindx)
        trgblog = (lf.mag[goodlog[goodlog2]])[logindx]
