...
;           conic = matrix##xy 		     ; rotate the ellipse
...
; print up the coordinates

        scrnsz = get_screen_size() ; user screen size
        xsz = scrnsz[0]/3.
        ysz = ratio*scrnsz[0]/3.
...
; generate a text file of results
        ...
        resfile = respath+objname+'_regions.txt'
        ...
        if strupcase(okay) eq 'Y' then begin
            ...
            printf, lun1, '# Object      : ', name
            printf, lun1, '# Date        : ', systime()
            printf, lun1, '# -------------------------------------'
            printf, lun1, '# Type        : ', type
            printf, lun1, '# Center      : ', '('+strn(center[0],format='(F5.2)')+"',"+$
              strn(center[1],format='(F5.2)')+"')"
            printf, lun1, '# Radius      : ', strn(rad,format='(F4.2)')+"'"
            printf, lun1, '# Major Axis  : ', strn(maj,format='(F4.2)')+"'"
            printf, lun1, '# Minor Axis  : ', strn(min,format='(F4.2)')+"'"
            printf, lun1, '# Angle       : ', strn(ang,format='(F5.1)')+' deg'
            printf, lun1, '# Eccentricity: ', strn(ecc,format='(F5.3)')
            printf, lun1, '# Upper Left  : ', '('+strn(ulxy[0],format='(F5.2)')+$
              "',"+strn(ulxy[1],format='(F5.2)')+"')"
            printf, lun1, '# Lower Right : ', '('+strn(lrxy[0],format='(F5.2)')+$
              "',"+strn(lrxy[1],format='(F5.2)')+"')"

            free_lun, lun1

        endif

        plotpath = path[3]          ; plots subdirectory
        plotfile = plotpath+objname+'_regions'

        okay = 'N' & print
        read, okay, prompt='Write the regions image to postscript (Y/[N])? ' & print
        if strupcase(okay) eq 'Y' then begin
            ps_open, plotfile, /ps_fonts, /portrait
            device, /times
            position = aspect(ratio,margin=0.17)
            plot, xcenter, ycenter, ps=3, color=7, $
              xsty=3, ysty=3, xtit = "x (arcmin)", ytit="y (arcmin)", $
              title = name, position=position
            oplot, xconic, yconic, color=5, thick=5
            ps_close
	endif
...
; generate a text file of results
        ...
        if strupcase(okay) eq 'Y' then begin
            ...
            position = aspect(ratio,margin=0.17)
            plot, xcenter, ycenter, ps=3, color=7, $
              xsty=3, ysty=3, xtit = "x position (')", ytit="y position (')", $
              title = name, position=position
            oplot, [x_ul,x_lr], [y_ul,y_ul], line=0, thick=5, color=5
            oplot, [x_lr,x_lr], [y_ul,y_lr], line=0, thick=5, color=5
            oplot, [x_ul,x_lr], [y_lr,y_lr], line=0, thick=5, color=5
            oplot, [x_ul,x_ul], [y_lr,y_ul], line=0, thick=5, color=5
            ps_close
	endif
...
pro regions_arbitrary
	...
	print & print, 'This option is not very well developed! ' & print
...
;        
;        file = path+objname+'_regions.ps'
;        print & print, 'Creating '+file+' . . .'
;
...
;        back = !p.background
;        set_plot, 'ps'
;        device, file=file, xoff=1.0, yoff=1.0, xsize=6.5, ysize=9.0, $
;        /portrait, /inches, /times
;        !p.font = 0             ; ps fonts
;        !p.background = 0.
;        tv, im
;        set_plot, 'x'
...
; -----------------------------------------------------------------------------
        ...
        plot, [xminmax[0],xminmax[1]], [yminmax[0],yminmax[1]], /nodata, $
          color=1, xsty=3, ysty=3, xtit = "x offset (arcmin)", ytit="y offset (arcmin)", $
          title = name, position=position
;       plotsym, 0, 0.25, /fill
...
;	ccut : option to read in data which has been color cut (TRGB_COLORCUT)
;
; OUTPUTS:
;	The user is prompted to write out a text file of the
;	parameters of the region selected
;	(/results/objname_region.txt), a postscript image of the
;	stellar positions and the region selected
;	(/plots/objname_region.ps), and the new photometry
;	(objname_halo.dat and objname_core.dat).  The first two files
;	are written to the path /deep1/ioannis/trgb while the
;	photometry goes into that object's specific subdirectory and
;	can be read in with TRGB_READATA.  All these paths are
;	specified in one place:  TRGB_DATAPATH.
;
; COMMON BLOCKS:
;	trgb, galaxy, haloindx, coreindx
;
; COMMENTS:
;	Please make sure you check the terminal for prompts -- they're
;	easy to miss!
;
;	The Arbitrary option of the widget has not been very well developed.
;
; PROCEDURES USED:
...
; read in the data for this object
        ...
        if keyword_set(hst) then begin
            ...
            axis = [1600.,1600.]	; axis length (arcmin)

            xcenter = data[1,*] * 0.1 / 60.
            ycenter = data[2,*] * 0.1 / 60.
