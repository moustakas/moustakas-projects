pro wise_zabs_vdisp, wave, flux, ferr, tempwave=tempwave, $
...
function wise_fit_continuum, wave, flux, ferr, tempwave=tempwave, $
...
function wise_fit_lines, wave, flux, ferr, continuum, smooth_continuum, $
...
function wise_smooth_residuals, wave, flux, continuum, $
...
; fit each object using GANDALF/PPXF
    t0 = systime(1)
;   for iobj = 82, nobj-1 do begin
    for iobj = 0, nobj-1 do begin
       print, format='("Fitting object ",I0,"/",I0,A10,$)', $
         iobj, nobj-1, string(13b)

; spectra are already logarithmically binned
       good = where(spec1d[iobj].invvar gt -1,npix)
;      good = where(spec1d[iobj].invvar ge 0,npix)
       wave = alog(10)*(dindgen(npix)*spec1d[iobj].cd1_1+spec1d[iobj].crval1) ; vacuum!
       junk = exp(wave) & vactoair, junk & wave = alog(junk)

; deredden
       dust = 10^(+0.4*spec1d[iobj].ebv_mw*k_lambda(exp(wave),/odonnell))

       flux = spec1d[iobj].flux[good]*dust/fluxscale
       invvar = spec1d[iobj].invvar[good]*fluxscale^2/dust^2
       ferr = 1.0/(sqrt(invvar)+(invvar eq 0))*(invvar ne 0)
       ferr = ferr+(ferr eq 0)*1D5
       continuum_snr = djs_median(flux*sqrt(invvar))
...
;          wise_zabs_vdisp, wave, flux, ferr, tempwave=tempwave, $
;            tempflux=tempflux1, velscale=velscale, zabs_guess=spec1d[iobj].z,$
;            vdisp_guess=spec1d[iobj].vdisp, alllinefile=alllinefile, final_zabs=zabs1,$
...
;          endif
          ...
          zabs1 = spec1d[iobj].z
          zabs_err1 = spec1d[iobj].z_err

; http://www.sdss.org/dr7/algorithms/veldisp.html
          if (spec1d[iobj].vdisp gt 70.0) and (spec1d[iobj].vdisp lt 420.0) and $
            (spec1d[iobj].sn_median gt 10) then begin
             vdisp1 = spec1d[iobj].vdisp
             vdisp_err1 = spec1d[iobj].vdisp_err
          endif else begin
             vdisp1 = 150.0
             vdisp_err1 = -1.0
          endelse
...
; get a better estimate of the absorption-line redshift
          continuum1 = wise_fit_continuum(wave,flux,ferr,tempwave=tempwave, $
...
; smooth the residuals before before fitting the lines
       smooth_continuum = wise_smooth_residuals(wave,flux,$
         continuum,zabs=zabs,velscale=velscale,broad=broad,$
         linefile=linefile,debug=debug)

; finally fit the emission lines
       sol = wise_fit_lines(wave,flux,ferr,continuum,smooth_continuum,$
         zabs=zabs,vdisp=vdisp,velscale=velscale,linefile=linefile,$
       ...
       if (total(sol gt 1D20) ge 1) then begin
          splog, 'Like a problem with object ', iobj
       endif
...
; pack everything into a structure          
       ...
       specdata1.isdss = spec1d[iobj].isdss
